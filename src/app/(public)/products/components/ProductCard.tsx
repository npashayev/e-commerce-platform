...
const ProductCard = ({ product }: Props) => {
  ...
  return (
    ...
    >
      <div className={styles.heading} title={product.brand || undefined}>
        <p className={styles.brand}>{product.brand}</p>

        {/* <LikeButton className={styles.likeBtn} product={product} /> */}
      </div>
      {discountPercentage > 0 && (
        <div className={styles.discountPercentage}>-{discountPercentage}%</div>
      )}
      ...
      <div className={styles.circleCnr}>
        {[...Array(imageLength)].map((_, i) => (
          <div
            key={i}
            className={`${styles.circle} ${i === activeIndex ? styles.activeCircle : ''}`}
          ></div>
        ))}
      </div>
        ...
        <div className={styles.titleCnr}>
          ...
          <div className={styles.ratingCnr}>
            ...
            <div
              ...
              style={{ width: `${(productRating / 5) * 60}px` }}
            >
              {[...Array(5)].map((_, i) => (
                <Image
                  key={i}
